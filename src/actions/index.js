...
const setModalVisibility = modalVisibility => dispatch =>
  dispatch({
    type: AppActions.SET_MODAL_VISIBILITY,
    payload: modalVisibility
  });

const getSearchGifs = searchText => dispatch => {
  getGifs(searchText)
    .then(response => {
      dispatch({
...
export { AppActions, updateSearchText, setModalVisibility, getSearchGifs };
